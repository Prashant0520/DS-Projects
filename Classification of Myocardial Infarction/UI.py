...
<style>
.stApp {
background-image: url("https://png.pngtree.com/back_origin_pic/00/03/46/bd474658399f3511d0f1b74d529e31cc.jpg");
...
# Loading model
with open(r"C:\Users\PRASH\Desktop\Project\Project2 Final Files\Classification.pkl", 'rb') as f:
...
def main():
    ...
    st.write('Your CSV file should have data as follows :')
    df = pd.read_csv(r"C:\Users\PRASH\Desktop\Project\Project2 Final Files\Input file format.csv")
    ...
    st.write("Here's information about the parameters in CSV file :")
    
    with open(r"C:\Users\PRASH\Desktop\Project\Project2 Final Files\Input file info.txt", 'r', encoding='utf-8') as file:
        content = file.read()
    ...
    if uploaded_file is not None:
        ...
        if selection == None:
            ...
            st.write('Outcome Cases Fractions : ')
            st.write('  0 is for Unknown (Alive),   1 is for Cardiogenic shock,   2 is for Pulmonary edema') 
            st.write('  3 is for Myocardial rupture,   4 is for Progress of congestive heart failure')
            st.write('  5 is for Thromboembolism,   6 is for Asystole,   7 is for Ventricular fibrillation')
            
            if st.button('More information'):
                @st.cache_data
                def calculate_value_counts():
                    value_counts = Input['Outcome'].value_counts()
                    return value_counts
                
                values = calculate_value_counts()
                st.write(values)
